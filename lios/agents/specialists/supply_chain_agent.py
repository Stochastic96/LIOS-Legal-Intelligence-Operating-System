"""Supply-chain specialist agent – CSDDD, CBAM, conflict minerals."""

from __future__ import annotations

from lios.agents.base_agent import AgentResponse, BaseAgent

_SYSTEM_PROMPT = """You are a specialist in EU supply-chain due-diligence law:
- CSDDD (Corporate Sustainability Due Diligence Directive 2024/1760)
- CBAM (Carbon Border Adjustment Mechanism 2023/956)
- Conflict Minerals Regulation (2017/821)
- EU Deforestation Regulation (2023/1115)

Given the regulatory context below and the user's question, respond in JSON:

{{
  "answer": "<concise legal answer>",
  "citations": [
    {{"regulation": "<short name>", "article": "<Art. X>", "excerpt": "<quoted text>"}}
  ],
  "confidence": <0.0-1.0>,
  "reasoning": "<chain of thought>"
}}

Regulatory context:
{context}

User question: {query}
"""


class SupplyChainAgent(BaseAgent):
    """Handles supply-chain due-diligence and CBAM queries."""

    agent_id = "supply_chain"

    async def respond(self, query: str, context_chunks: list[str]) -> AgentResponse:
        prompt = _SYSTEM_PROMPT.format(
            context=self._format_context(context_chunks),
            query=query,
        )
        raw = await self._call_llm(prompt)
        parsed = self._parse_json_response(raw)

        return AgentResponse(
            agent_id=self.agent_id,
            answer=parsed.get("answer", raw),
            citations=parsed.get("citations", []),
            confidence=float(parsed.get("confidence", 0.8)),
            reasoning=parsed.get("reasoning", ""),
        )
